...
class CLIService {
    ...
    async startGateway() {
        ...
        if (!success)
            return { success: false, error: error || '启动网关失败' };
        for (let i = 0; i < 10; i++) {
    ...
    async stopGateway() {
        ...
        if (!success)
            return { success: false, error: error || '停止网关失败' };
        return { success: true };
    }
    ...
    async setConfig(key, value) {
        ...
        if (!success)
            return { success: false, error: error || '设置配置失败' };
        return { success: true };
    }
    // 平台管理
    async getPlatforms() {
        ...
        for (const platform of platforms) {
            ...
            platform.enabled = !!platformConfig?.enabled || envEnabled;
    ...
    async setPlatformToken(platform, token) {
        ...
        try {
            if (!this.config?.envPath)
                return { success: false, error: '环境变量路径未初始化' };
            let content = '';
            try {
