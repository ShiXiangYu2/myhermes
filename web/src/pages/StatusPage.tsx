...
export default function StatusPage() {
  ...
  const handleStartGateway = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : '启动网关失败');
    } finally {
  ...
  const handleStopGateway = async () => {
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error || '停止网关失败');
      } else {
        const newStatus = await api.getStatus();
        setStatus(newStatus);
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : '停止网关失败');
    } finally {
      setActionInProgress(null);
    }
  };
  ...
  const handleRestartGateway = async () => {
    ...
    try {
      ...
      const result = await api.startGateway();
      if (!result.success) {
        setError(result.error || '重启网关失败');
      } else {
        const newStatus = await api.getStatus();
        setStatus(newStatus);
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : '重启网关失败');
    } finally {
      setActionInProgress(null);
    }
  };
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-base">网关控制</CardTitle>
            </div>
            <div className="flex gap-2">
              {status.gateway_running ? (
                <>
                  ...
                  >
                    <RefreshCw className={`h-3.5 w-3.5 ${actionInProgress === 'restart' ? 'animate-spin' : ''}`} />
                    重启
                  </button>
                  <button
                    onClick={handleStopGateway}
                    disabled={actionInProgress !== null}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-destructive/10 text-destructive border border-destructive/30 rounded-md hover:bg-destructive/20 transition-colors disabled:opacity-50"
                  >
                ...
                >
                  ...
                  {actionInProgress === 'start' ? '启动中...' : '启动网关'}
                </button>
              )}
            </div>
          </div>
